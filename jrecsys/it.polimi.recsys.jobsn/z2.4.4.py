...
print ('Executing z2.4.4.py at ', datetime.datetime.now())
...
def getRecommendations(target_user_id) :
    ...
    user_bias = pd.DataFrame(((jobs_similar_users.groupby(['user_id'])['interaction_type'].sum())/(jobs_similar_users.groupby(['user_id']).size() + 50))).reset_index()
    user_bias.columns = ['user_id','avg']
    jobs_similar_users = pd.merge(jobs_similar_users,user_bias, on='user_id')[['user_id','item_id','jaccard_index','interaction_type', 'popularity','avg']]
    jobs_similar_users['avg'] = jobs_similar_users['interaction_type'] - jobs_similar_users['avg']
    

    #numerador
    jobs_similar_users['numerador'] =  jobs_similar_users['jaccard_index'] * jobs_similar_users['avg']
    ranking = pd.DataFrame((jobs_similar_users.groupby(['item_id'])['numerador'].sum())).reset_index()
    denominador = jobs_similar_users.drop_duplicates(['user_id'], keep='first')['jaccard_index'].sum()
    avg_user = jobs_target_user_did_interact_with['interaction_type'].sum()/(len(jobs_target_user_did_interact_with) + 50)
    ranking['rank'] = avg_user + ranking['numerador']/denominador
    ranking = ranking.sort_values('rank',ascending=False)
    
#    if ranking[:5].empty:
#        return fillBlanks(target_user_id)
#    else:
    return '' + ranking[:5]['item_id'].astype(str).str.cat(sep=' ')
